<template>
	<view class="container">
		<view class="customHead">
			<view @click="$tools.back(1)">
				< </view>
					<view class="title">
						{{$t('pledge.pledgeinfo')}}
					</view>
					<view class="tools" @click="allearming">
						{{$t('pledge.yjlq')}}
					</view>
			</view>
						...
						<view class="flex-col list-item" :key="i" v-for="(item, i) in pledgeList">
							...
							<view class="flex-row justify-between group_4">
								<view class="flex-row space-x-10">
									<text class="font_5 text_6">{{$t('pledge.pledgeweek')}}</text>
									<text class="font_6 text_7">{{item.time}}{{$t('index.day')}}</text>
								</view>
								<view class="flex-row items-baseline space-x-10">
									<text class="font_5">{{$t('pledge.rate')}}</text>
									<text class="font_7 text_8">{{item.rate}}%</text>
								</view>
							</view>
							<view class="flex-row group_5 view space-x-10">
								<text class="font_5 text_9">{{$t('pledge.pledgetime')}}</text>
								<text class="font_6">{{item.createTime}}</text>
							</view>
							<view class="flex-row items-center group_5 view_2 space-x-9">
								<text class="font_5">{{$t('pledge.earnings')}}(LTC)</text>
								<text class="font_6">{{item.earnings}}</text>
								<image class="image_4" src="../../static/logo.png" />
							</view>
							<view class="flex-col justify-start items-center button" style="background-color: #1e5676;"
								@click="getRedeem(item)" v-if="item.status == 4"><text class="font_1">{{$t('pledge.redeem')}}</text></view>
							<view class="flex-col justify-start items-center button" @click="getEarning(item)" v-else>
								<text class="font_1">{{$t('pledge.get')}}</text>
							</view>
						</view>
	...
	export default {
		...
		methods: {
			...
			async allearming(){
				let that = this;
				let studs = false
				that.pledgeList.forEach(item => {
					if (item.status == 3 && item.earnings > 0) {
						studs = true
					}
				})
				if(!studs){
					return that.$tools.toast('當前暫無訂單有收益可領取~')
				}
				const signkey = await that.$tools.signMessage('receive award')
				try {
					const signres = await this.$tools.verifyMessage({
