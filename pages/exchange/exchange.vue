		...
		<view class="flex-col flex-auto group space-y-28">
		  <view class="flex-col group_2">
		    <view class="flex-row justify-between items-center section">
			  <input class="text_3 ipttext" type="number" v-model="lttnum" placeholder="請輸入您要兌換的數量" />
		      <text class="font_2">LTT</text>
		    </view>
		    <image
		      class="self-center image_4 image_5"
		    ...
		    <view class="flex-row justify-between section view">
			  <input class="text_3 ipttext" type="number" v-model="ltcnum" disabled="true" placeholder="可兌換的數量">
		  ...
		  <view class="flex-col justify-start items-center button" @click="$tools.throttle(exchangefuc,1000)"><text class="font_1">兑换</text></view>
		</view>
	</view>
</template>
		...
		watch: {
			lttnum(newval, oldval) {
				...
				if(newval > that.lttinfo.lttUsing){
					...
					return this.$tools.toast('不能超過自己的餘額~')
				}
